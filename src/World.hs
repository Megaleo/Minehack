...
chunkMap mult perlin_ coord = [round $ mult * (twoD (toEnum x) (toEnum y) perlin_) | (x, y) <- range $ chunkRange coord]
...
genPerlinChunk                           :: Double -> Perlin -> ChunkCoord -> (Int -> T.Tile) -> [T.Tile]
genPerlinChunk mult perlin_ cCoord pFunc = map pFunc $ chunkMap mult perlin_ cCoord